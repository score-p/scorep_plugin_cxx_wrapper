...
#define ENV_CONFIG_PREFIX "SCOREP_METRIC_SINE_STRICT_PLUGIN_"
...
class sine_strict_plugin
    : public scorep::plugin::base<sine_strict_plugin, spp::per_thread,
                                  spp::sync_strict, spp::scorep_clock,
                                  sine_object_id>
{
public:
    ...
    {
        logging::info() << "adding sine strict metric with interval "
                        << m.interval;
    }
    ...
    template <typename P>
    void get_current_value(sine_metric& m, P& proxy)
    {
        auto value = m.value();
        if (value >= 0)
        {
...
SCOREP_METRIC_PLUGIN_CLASS(sine_strict_plugin, "sine_strict")
